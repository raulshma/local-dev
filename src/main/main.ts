...
const addScriptExecutionHistory = (historyEntry: ScriptExecutionHistory) => {
  const projectHistory =
    scriptExecutionHistory.get(historyEntry.projectId) || [];
...
const getScriptExecutionHistory = (
  projectId: string,
): ScriptExecutionHistory[] => {
...
const killProcessTree = async (pid: number): Promise<boolean> => {
  try {
    if (process.platform === 'win32') {
      ...
      console.log(
        `Found ${allPids.size} processes to terminate:`,
        Array.from(allPids),
      );
...
const setupIpcHandlers = () => {
  ...
  ipcMain.handle(
    'project:add',
    async (event, name: string, projectPath: string) => {
      try {
        ...
        try {
          const detectedTypes =
            await projectDetectionService.detectProjectType(projectPath);
          const detectedScripts = await projectDetectionService.generateScripts(
            projectPath,
            detectedTypes,
          );
  ...
  ipcMain.handle('project:detect-scripts', async (event, projectId: string) => {
    try {
      ...
      const detectedTypes = await projectDetectionService.detectProjectType(
        project.path,
      );
      const detectedScripts = await projectDetectionService.generateScripts(
        project.path,
  ...
  ipcMain.handle(
    ...
    async (event, projectId: string) => {
      try {
        ...
        const success = storeService.addAutoDetectedScripts(
          projectId,
          detectedScripts,
        );
  ...
  ipcMain.handle('project:start-dev', async (event, projectId: string) => {
    try {
      ...
      const devCommand = await projectDetectionService.findBestDevCommand(
        project.path,
      );
      if (!devCommand) {
        return {
      ...
      console.log(
        `Attempting to start dev server for project: ${project.name}`,
      );
      ...
      if (runningProcesses.has(processKey)) {
        ...
        console.log(
          `Stopping existing dev server process for project: ${projectId}`,
        );
      ...
      console.log(
        `Parsed dev command: ${command}, args: ${JSON.stringify(args)}`,
      );
      ...
      if (process.platform === 'win32') {
        ...
        try {
          ...
          if (!childProcess.pid) {
            throw new Error(
              'Failed to start dev server process: No PID obtained',
            );
          }

          console.log(
            `Successfully started dev server process with PID: ${childProcess.pid}`,
          );
        } catch (windowsSpawnError) {
          console.error(
            'Error spawning dev server on Windows:',
            windowsSpawnError,
          );
          throw new Error(
            `Failed to start dev server on Windows: ${windowsSpawnError instanceof Error ? windowsSpawnError.message : 'Unknown error'}`,
          );
        }
      } else {
        ...
        try {
          ...
          console.error(
            'Error spawning dev server on non-Windows:',
            nonWindowsSpawnError,
          );
          throw new Error(
            `Failed to start dev server: ${nonWindowsSpawnError instanceof Error ? nonWindowsSpawnError.message : 'Unknown error'}`,
          );
        }
      }
  ...
  ipcMain.handle(
    ...
    async (event, projectId: string, scriptId: string) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (event, projectId: string, scriptId: string) => {
      try {
        ...
        console.log(
          `Attempting to execute script "${script.name}" (${scriptId}) for project: ${project.name}`,
        );
        ...
        console.log(
          `Parsed command: ${command}, args: ${JSON.stringify(args)}`,
        );
        ...
        if (process.platform === 'win32') {
          ...
          try {
            ...
            if (!childProcess.pid) {
              throw new Error(
                'Failed to start script process: No PID obtained',
              );
            }

            console.log(
              `Successfully started script process with PID: ${childProcess.pid}`,
            );
          } catch (windowsSpawnError) {
            console.error(
              'Error spawning script on Windows:',
              windowsSpawnError,
            );
            throw new Error(
              `Failed to execute script on Windows: ${windowsSpawnError instanceof Error ? windowsSpawnError.message : 'Unknown error'}`,
            );
          }
        } else {
          ...
          try {
            ...
            console.error(
              'Error spawning script on non-Windows:',
              nonWindowsSpawnError,
            );
            throw new Error(
              `Failed to execute script: ${nonWindowsSpawnError instanceof Error ? nonWindowsSpawnError.message : 'Unknown error'}`,
            );
          }
        }
        ...
        childProcess.stdout?.on('data', (data) => {
          ...
          const latestExecution = projectHistory.find(
            (h) => h.scriptId === scriptId && h.status === 'running',
          );
          if (latestExecution) {
        ...
        childProcess.on('close', (code, signal) => {
          console.log(
            `Script ${scriptId} closed with code: ${code}, signal: ${signal}`,
          );
          runningProcesses.delete(processKey);
          ...
          if (latestExecution) {
            ...
            const duration =
              Date.now() - new Date(latestExecution.startTime).getTime();
            updateScriptExecutionHistory(projectId, latestExecution.id, {
        ...
        childProcess.on('exit', (code, signal) => {
          console.log(
            `Script ${scriptId} exited with code: ${code}, signal: ${signal}`,
          );
          if (runningProcesses.has(processKey)) {
            runningProcesses.delete(processKey);
  ...
  ipcMain.handle(
    ...
    async (event, projectId: string, scriptId: string) => {
      try {
        ...
        console.log(
          `Attempting to stop script: ${scriptId} for project: ${projectId}`,
        );
        ...
        console.log(
          `Terminating process tree with PID: ${pid} for script: ${scriptId}`,
        );
        ...
        try {
          ...
          const gracefulWaitPromise = new Promise<void>((resolve) => {
            const waitTimeout = setTimeout(() => {
              if (!gracefulShutdown) {
                console.log(
                  `Graceful shutdown timeout for script: ${scriptId}`,
                );
              }
              resolve();
        ...
        if (hasLivingDescendants) {
          console.log(
            `Found ${allDescendantPids.size} descendant processes that need to be killed:`,
            Array.from(allDescendantPids),
          );
        }
        ...
        if (!gracefulShutdown || processStillExists || hasLivingDescendants) {
          console.log(
            `Force killing process tree for script: ${scriptId} (graceful: ${gracefulShutdown}, parent exists: ${processStillExists}, descendants: ${hasLivingDescendants})`,
          );
          try {
              ...
              for (const descendantPid of allDescendantPids) {
                exec(`taskkill /F /PID ${descendantPid}`, (error) => {
                  if (
                    ...
                    console.log(
                      `Failed to kill descendant PID ${descendantPid}:`,
                      error.message,
                    );
                  }
                });
          ...
          console.log(
            `Script ${scriptId} terminated gracefully with no remaining processes`,
          );
        }
  ...
  ipcMain.handle(
    'env:save',
    async (event, projectId: string, variables: Record<string, string>) => {
      try {
        const project = storeService.getProject(projectId);
        if (!project) {
          throw new Error('Project not found');
  ...
  ipcMain.handle('action:open-ide', async (event, projectId: string) => {
    try {
      ...
      console.log(
        `Attempting to open project at ${projectPath} with IDE command: ${ideCommand}`,
      );
      ...
      if (process.platform === 'win32') {
        ...
        try {
          console.log(
            `Attempting to open IDE: ${resolvedIDECommand} with path: ${projectPath}`,
          );
          ...
          if (ideProcess.pid) {
            ideProcess.unref();
            console.log(
              `Successfully launched IDE process with PID: ${ideProcess.pid}`,
            );
            return {
              success: true,
              message: 'IDE launched using cmd start with clean environment.',
            };
          }
          ...
          return {
            success: true,
            message:
              'Opened project folder with default application (IDE command failed)',
          };
        }
      } else {
        ...
        console.log(
          `Attempting to open project on non-Windows platform with: ${ideCommand} ${projectPath}`,
        );
        try {
          ...
          if (ideProcess.pid) {
            ideProcess.unref();
            console.log(
              `Successfully launched IDE process with PID: ${ideProcess.pid} on non-Windows`,
            );
            return { success: true };
          }
          throw new Error(
            'Failed to start IDE process on non-Windows: No PID obtained.',
          );
  ...
  ipcMain.handle('action:open-terminal', async (event, projectId: string) => {
    try {
      ...
      console.log(
        `Opening terminal for project: ${project.name} at path: ${projectPath}`,
      );
        ...
        try {
          if (platform === 'win32') {
            ...
            const terminalCommand =
              effectiveSettings.terminalCommand || 'powershell.exe';
            ...
            setTimeout(() => {
              try {
                ...
                resolve({
                  success: false,
                  error:
                    'Failed to start terminal. Make sure the terminal application is installed.',
                });
              }
            }, 1000);
          } else {
            ...
            if (effectiveSettings.terminalCommand) {
              ...
              command =
                terminals[platform as keyof typeof terminals] ||
                terminals.default;
            }
    ...
    async (event, settings: Partial<AppSettings>) => {
      try {
        ...
        if ((settings as any).notifications) {
          trayService.updateNotificationSettings(
  ...
  ipcMain.handle(
    ...
    async (
      event,
      projectId: string,
      settings: Partial<{ ideCommand: string; terminalCommand: string }>,
    ) => {
      try {
    ...
    try {
      ...
      for (const filename of envFiles) {
        ...
        if (existsSync(envPath)) {
          ...
          let existingConfig = envMetadata.configurations.find(
            (c: EnvironmentConfiguration) => c.filename === filename,
          );
          if (!existingConfig) {
      ...
      if (!envMetadata.activeConfigId && foundConfigurations.length > 0) {
        ...
        const defaultConfig =
          foundConfigurations.find((c) => c.filename === '.env') ||
          foundConfigurations[0];
  ...
  ipcMain.handle(
    'env-config:switch',
    async (event, projectId: string, configId: string) => {
      try {
        const project = storeService.getProject(projectId);
        if (!project) {
          throw new Error('Project not found');
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const config = envMetadata.configurations.find(
          (c: EnvironmentConfiguration) => c.id === configId,
        );
        if (!config) {
  ...
  ipcMain.handle(
    ...
    async (event, projectId: string, configId: string) => {
      try {
        ...
        envMetadata.configurations = envMetadata.configurations.filter(
          (c) => c.id !== configId,
        );

        // If this was the active configuration, switch to another one
        if (envMetadata.activeConfigId === configId) {
          envMetadata.activeConfigId =
            envMetadata.configurations.length > 0
  ...
  function getDisplayNameForEnvFile(filename: string): string {
    ...
    return (
      nameMap[filename] ||
      filename.replace('.env.', '').replace('.env', 'Custom')
    );
  }
  ...
  ipcMain.handle(
    ...
    async (event, projectId: string, limit: number = 10) => {
      try {
        const project = storeService.getProject(projectId);
        if (!project) {
          throw new Error('Project not found');
  ...
  ipcMain.handle(
    'git:stage-files',
    async (event, projectId: string, files: string[]) => {
      try {
        const project = storeService.getProject(projectId);
        if (!project) {
          throw new Error('Project not found');
  ...
  ipcMain.handle(
    'git:commit',
    async (event, projectId: string, message: string) => {
      try {
        const project = storeService.getProject(projectId);
        if (!project) {
          throw new Error('Project not found');
  ...
  ipcMain.handle(
    'git:get-diff',
    async (event, projectId: string, filePath?: string) => {
      try {
        const project = storeService.getProject(projectId);
        if (!project) {
          throw new Error('Project not found');
  ...
  ipcMain.handle(
    'git:apply-stash',
    async (event, projectId: string, stashIndex: number) => {
      try {
        const project = storeService.getProject(projectId);
        if (!project) {
          throw new Error('Project not found');
  ...
  ipcMain.handle(
    ...
    async (event, projectId: string) => {
      try {
        ...
        const config = await DockerService.detectDockerConfiguration(
          project.path,
        );
  ...
  ipcMain.handle(
    ...
    async (event, projectId: string) => {
      try {
        ...
        const scripts = DockerService.generateDockerScripts(
          project.path,
          config,
        );
  ...
  // Window control handlers (must be after imports)
  ...
  ipcMain.handle(
    'terminal:execute',
    async (event, id: string, command: string) => {
      try {
  ...
  ipcMain.handle(
    'terminal:cd',
    async (event, id: string, directory: string) => {
      try {
